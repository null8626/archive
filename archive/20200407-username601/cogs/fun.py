...
class fun(commands.Cog):
    ...
    async def edit(self, ctx, *args):
        ...
        msg = await ctx.reply('...')
        await msg.edit(res[0] + '\u202b '+ res[1] + ' \u202b')

    @command(['echo'])
    @cooldown(5)
    @require_args()
    async def say(self, ctx, *args):
        return await ctx.reply(' '.join(args))
    ...
    async def mock(self, ctx, *args):
        ...
        return await ctx.reply(''.join([choice([i.upper(), i.lower()]) for i in list(text)]))
    ...
    async def choose(self, ctx, *args):
        ...
        return await ctx.reply(choice(' '.join(args).split(',')))
