...
class utils(commands.Cog):
    ...
    @cooldown(5)
    @require_args()
    async def newgrounds(self, ctx, *args):
        try:
            number = int(args[0])
            assert number in range(0, 1000000)
            if number not in self._song_cache:
                response = await ctx.bot.http._HTTPClient__session.post("http://boomlings.com/database/getGJSongInfo.php", data={ "secret": "Wmfd2893gb7", "songID": number })
                response = await response.text()
                if response.lstrip("-").isnumeric():
                    self._song_cache[number] = None
                    raise ctx.error_message(f"The song with the ID `{number}` does not exist.", description=True)
                
                try:    title = findall(r"\~2\~\|\~(.*?)\~\|\~", response)[0]
                except: title = "Untitled"
                try:    author = findall(r"\~4\~\|\~(.*?)\~\|\~", response)[0]
                except: author = "`???`"
                try:    size = float(findall(r"\~5\~\|\~(.*?)\~\|\~", response)[0])
                except: size = 0.0
                try:    download = unquote(findall(r"\~10\~\|\~(.*?)\~\|\~", response)[0])
                except: download = "https://google.com/"
                self._song_cache[number] = (title, author, size, download)
                del response
            else:
                if not self._song_cache[number]:
                    raise ctx.error_message(f"The song with the ID `{number}` does not exist.", description=True)
                title, author, size, download = self._song_cache[number]
            
            await ctx.embed(title=title, url=f"https://newgrounds.com/audio/listen/{number}", fields={
                "Song ID": number,
                "Author": author,
                "Size": f"{size} MB",
                "Download Link": f'[Click Here]({download})'
            })
            
            del title, author, size, download, number
    ...
    async def country(self, ctx, *args):
        ...
        try:
            ...
            data = await ctx.bot.util.request(f'https://restcountries.eu/rest/v2/name/{quote(_country)}', json=True)
    ...
    async def nasa(self, ctx, *args):
        query = quote(' '.join(args) or 'earth')
        await ctx.trigger_typing()
    ...
    async def calc(self, ctx, *args):
        ...
        try:
            res = eval(equation)
            return await ctx.reply(f"{equation} = `{str(res)[:1000]}`")
        except Exception as e:
