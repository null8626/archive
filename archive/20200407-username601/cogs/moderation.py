...
class moderation(commands.Cog):
    ...
    async def joinposition(self, ctx, *args):
        ...
        return await ctx.reply(embed=discord.Embed(title=title, description=desc, color=ctx.me.color))
    ...
    async def role(self, ctx, *args):
        if 'member' in args[0].lower():
            ...
            if isinstance(role, list):
                ...
                role = await choose.run()
                if not role:
                    return
                del choose
            
            _map = [i.mention for i in role.members]
            paginator = ctx.bot.EmbedPaginator.from_long_array(ctx, _map, {
                "title": f"{role.name}'s Members [{len(role.members):,}]",
                "thumbnail": { "url": str(ctx.guild.icon_url) }
            }, char=", ", max_char_length=1000, max_pages=5)
            
            if not paginator:
                await ctx.embed(title=f"{role.name}'s Members [{len(role.members):,}]", thumbnail=ctx.guild.icon_url, description=", ".join(_map))
                del _map, role, paginator
            else:
                del _map, role
            ...
            del extra, role, permissions
            return
        elif "list" in args[0].lower():
