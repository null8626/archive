...
class ImageClient:
    ...
    def __init__(self, client):
        ...
        for frame in ImageSequence.Iterator(Image.open("./assets/pics/explosion.gif")):
            frame = frame.copy()
            self.EXPLOSION_GIF.append(frame)
    ...
    async def image_from_URL(self, url: str):
        ...
        headers = (res.headers.get("content-type") or res.headers.get("Content-Type", "")).lower()
        if headers and headers.startswith("image/svg"):
            return self._resolve_svg(byte_data, True)
        
        image = Image.open(BytesIO(byte_data))
        del byte_data, res, headers
    ...
    async def explode_animated(self, url: str, reversed: bool = False):
        ...
        temp_images = []
        base_im = await self.wand_from_URL(url)
        base_im.resize(112, 112)
        
        for i in range(1, 11):
            im = _Image(image=base_im)
            im.implode(amount=(i * 0.15 if reversed else -i))
            im_ = Image.open(BytesIO(im.make_blob("png")))
            temp_images.extend([im_.copy() for _ in range(3)])
            im_.close()
            del im_, im
        base_im.close()
        del base_im
        
        temp_images += self.EXPLOSION_GIF
        buffer = BytesIO()
...
class GDLevel:
    ...
    async def draw(self):
        resp = await self.bot.http._HTTPClient__session.get('https://gdbrowser.com/api/level/' + quote(self.query))
        data = await resp.json()
        
        levelName = data.get('name', 'undefined')
        levelAuth = "by "+data.get('author', '-')
        levelDesc = data.get('description', 'This level is without description.')
        levelDiff = data.get('difficulty', 'N/A')
        levelStars = f"{data.get('stars', 69):,} stars"
        ...
        desc_cursor = 300
        texts, sym_cursor = self.bot.Image.wrap_text(levelDesc, self.aller, 445), 100
        ...
        difficulty = await self.bot.Image.image_from_URL(f"https://gdbrowser.com/assets/difficulties/{data.get('difficultyFace', 'unrated')}.png")
...
class UserCard:
    ...
    async def send(self):
        ...
        big_font =  ImageFont.truetype(self.font_path, 40)
        smol_font = ImageFont.truetype(self.font_path, 25)
        tiny_font = ImageFont.truetype(self.font_path, 20)
        ...
        main = Image.new(mode="RGB", size=(width, 190), color=background_color)
        draw = ImageDraw.Draw(main)
...
class ServerCard:
    def __init__(self, ctx, font_path: str):
        ...
        self._server_tier_urls = [
            "https://removed.is-inside.me/YWs1Qw2q.png",
            "https://removed.is-inside.me/BIe3ccYA.png",
            "https://removed.is-inside.me/2vKUVwwM.png",
            "https://removed.is-inside.me/T0sfEdj1.png"
        ]
