...
class GuessTheFlag:
    ...
    async def start(self):
        ...
        message = await self.ctx.send_image(f"https://www.countryflags.io/{self.flag}/shiny/64.png", content="Guess the country that has this flag!")
...
class Hangman:
    ...
    async def play(self):
        nl = "\n"
        main_message = await self.ctx.send(embed=Embed(title="Hangman", description=f"**{' '.join(self.blacklisted)}**\nWrong words: <none>\nTurns left: `{8 - self.tries}`", color=self.ctx.me.color).set_image(url=f"https://raw.githubusercontent.com/null8626/username601/master/assets/pics/hangman_{self.tries}.png"))
        ...
        while self.tries <= 7:
            ...
            if "\_" not in self.blacklisted:
                await self.ctx.send(f"{self.ctx.author.mention}, You are correct! The answer is ***{self.word}***.")
                return True
        ...
        await self.ctx.send(f"{self.ctx.author.mention}, You lost the game. The answer is ***{self.word}***.")
        return False
...
class GuessMyNumber:
    ...
    async def play(self, ctx) -> bool:
        ...
        while self.rounds_left != 0:
            ...
            if not response:
                await ctx.reply(f"{ctx.author.display_name} went AFK. So i closed the game.")
                return

            if int(response.content) == self.my_number:
                await ctx.reply(f"You are correct! my number is {self.my_number}")
                return True

            await ctx.reply("Lower!" if (int(response.content) > self.my_number) else "Higher!")
            self.rounds_left -= 1
        await ctx.reply(f"You lost! My number is actually {self.my_number}!")
        return False
...
class Trivia:
    ...
    async def generate_question(self, ctx) -> dict:
        result = await self.session.get(f"https://wiki-quiz.herokuapp.com/v1/quiz?topics={quote(self.topic)}&limit=10")
...
class GuessAvatar:
    ...
    async def start(self) -> bool:
        ...
        response = await embed.send()
        message = Message(state=embed.ctx._state, channel=self.ctx.channel, data=response)
        del embed, response
...
class RockPaperScissors:
    ...
    async def _is_win(self, user_index: int, bot_index: int):
        ...
        if pos in [(0, 2), (1, 0), (2, 1)]:
            await self.message.edit(embed=Embed(title=f"Congratulations, {self.ctx.author.display_name} won against {self.ctx.bot.user.name}!", description=f"**{self.ctx.author.display_name}: **{self.emojis[user_index]}\n**{self.ctx.bot.user.name}: **{self.emojis[bot_index]}", color=Color.green()))
            return 1
        elif pos in [(0, 0), (1, 1), (2, 2)]:
            await self.message.edit(embed=Embed(title=f"It's a draw!", description=f"**{self.ctx.author.display_name}: **{self.emojis[user_index]}\n**{self.ctx.bot.user.name}: **{self.emojis[bot_index]}", color=Color.orange()))
            return 0
        await self.message.edit(embed=Embed(title=f"RIP, {self.ctx.author.display_name} lost to {self.ctx.bot.user.name}!", description=f"**{self.ctx.author.display_name}: **{self.emojis[user_index]}\n**{self.ctx.bot.user.name}: **{self.emojis[bot_index]}", color=Color.red()))
        return -1
